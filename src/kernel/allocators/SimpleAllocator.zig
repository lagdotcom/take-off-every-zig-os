...
const log = std.log.scoped(.simple_alloc);
...
pub const SimpleAllocator = @This();

first: *Entry,

pub fn init(blocks: []const types.MemoryBlock) SimpleAllocator {
    ...
    return SimpleAllocator{ .first = first.? };
}

pub fn allocator(self: *SimpleAllocator) std.mem.Allocator {
    return .{
        .ptr = self,
        .vtable = &.{
            .alloc = alloc,
...
pub fn alloc(ctx: *anyopaque, n: usize, log2_ptr_align: u8, return_address: usize) ?[*]u8 {
    ...
    const self: *SimpleAllocator = @alignCast(@ptrCast(ctx));
...
pub fn debug(self: *SimpleAllocator) void {
    var entry: ?*Entry = self.first;
...
pub fn report(self: *const SimpleAllocator) types.UsageReport {
    var entry: ?*Entry = self.first;
...
pub fn report_table(self: *const SimpleAllocator, t: *shell.Table) !void {
    ...
    while (entry) |e| {
        ...
        try t.add_size(e.size);
        try t.add_size(e.reserved);
...
test "allocation" {
    ...
    var kalloc = SimpleAllocator.init(&.{
...
test "block splitting and joining" {
    ...
    var kalloc = SimpleAllocator.init(&.{.{ .addr = @intFromPtr(blk.ptr), .size = blk.len }});
    var a = kalloc.allocator();
